...
const SignUp = () => {
    ...
    const [formData, setFormData] = useState({ firstName: '', lastName: '', email: '', password: '', confirm_password: '', company: '' });
    const [errors, setErrors] = useState([])
    const [signupOpen, setsignupOpen] = useState(true);
    const [passwordVisible, setPasswordVisible] = useState(false);
    const [confirmPasswordVisible, setConfirmPasswordVisible] = useState(false);
    const { firstName, lastName, email, password, confirm_password, company } = formData
    ...
    const handleSubmit = async (event) => {
        ...
        if (Object.keys(newErrors).length === 0) {
            ...
            try {
                ...
                navigate("/verify-otp", { state: { email: formData.email, from: 'signup' } });
            ...
            if (exists) {
                if (passwordNull) {
                    ...
                    if (token) {
                        ...
                        localStorage.setItem('qs', encryptedToken);
                        navigate('/dashboard');
                    } else {
    ...
    const handleGoogleSubmit = async () => {
        ...
        try {
            ...
            const encryptedToken = CryptoJS.AES.encrypt(response.data.data, secretKey).toString();
            localStorage.setItem('qs', encryptedToken);
            setOpen(false);
            navigate('/dashboard');
