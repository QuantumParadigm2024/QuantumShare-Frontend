...
const AccountOverview = () => {
    ...
    const [profile, setProfile] = useState(null);
    const [open, setOpen] = useState(false);
    const [tempData, setTempData] = useState({ email: '', firstname: '', lastname: '', company: '' });
    const [errors, setErrors] = useState({});
    let navigate = useNavigate();
    const [isSessionExpired, setIsSessionExpired] = useState(false);
    ...
    const handleSaveChanges = async () => {
        ...
        if (!formData.has('firstname') && !formData.has('lastname') && !formData.has('company') && !formData.has('file') && !formData.has('email')) {
            toast.error("No changes to update.");
            setLoading(false);
            return;
        }
    ...
    return (
        <>
            <div>
                <Nav />
                <Dialog open={showPopup} onClose={(event, reason) => {
                    if (reason !== 'backdropClick') {
                        setShowPopup(false);
                    }
                    ...
                    <DialogActions sx={{ display: 'flex', flexDirection: 'column', gap: 1 }}>
                        <Link to='/pricing' style={{ textDecoration: 'none', width: '50%' }}>
                            <Button variant="contained" fullWidth sx={{ color: '#ba343b', background: "#fcf8f8", border: '1px solid #ba343b', borderRadius: '30px', fontWeight: 'bold', '&:hover': { background: 'none' } }} >
                                Subscribe Now
                            </Button>
                        </Link>
                    </DialogActions>
                ...
                <div style={{ display: 'flex' }}>
                        ...
                        {loading ? (
                            ...
                            userData && (
                                <div id='accountDisplay'>
                                    ...
                                    <button className="editBtn" onClick={handleClickOpen}> Edit <BorderColorOutlinedIcon sx={{ marginBottom: '-1px', color: 'white', fontSize: '18px' }} /> </button>
                                </div>
                            )
                        )}
                    </div>
                    <div>
                        <Dialog open={open} onClose={handleClose}>
                            <DialogTitle sx={{ textAlign: 'center' }}>Edit Profile <IoMdClose onClick={handleClose} style={{ position: 'absolute', right: '10px', top: '10px', cursor: 'pointer', color: '#ba343b' }} /> </DialogTitle>
                            <DialogContent className="dialog-content">
                                <div className="avatar-container-in">
                                    {profile ? (
                                        <img src={profile} alt="" className="avatar-in" />
                                    ...
                                    <div style={{ cursor: 'pointer', marginTop: '3px', display: 'block', textAlign: 'center', fontSize: 12, color: '#ba343b', fontWeight: '600', }} onClick={() => fileInputRef.current.click()}>Change Photo </div>
                                    <input type='file' ref={fileInputRef} style={{ display: 'none' }} accept='image/*' onChange={handleFileChange} />
                                </div>
                                <Divider sx={{ width: '450px', position: 'relative', top: '10px' }} />
                                <div id="scroll">
                            ...
                            <DialogActions>
                                <button onClick={handleSaveChanges} style={{ backgroundColor: '#ba343b', color: '#fff', padding: '10px 20px', borderRadius: '10px', border: 'none', cursor: 'pointer', fontSize: '14px', fontWeight: '600', margin: '10px' }}>
                                    {loading ? (
                                        <TailSpin height="20" width="20" color="#fff" ariaLabel="loading" />
                                    ) : (
                                        'Save Changes'
            ...
            <Dialog open={isSessionExpired} aria-labelledby="alert-dialog-title" PaperProps={{ sx: { backgroundColor: '#ffffff', width: '40vw', height: '30vh' } }}>
                <DialogContent sx={{ backgroundColor: '#ffffff' }}>
                    <DialogContentText sx={{ color: 'black', display: 'flex', fontSize: '20px', alignItems: 'center' }}>
                        <IconButton>
                            <WarningIcon style={{ color: 'orange', cursor: 'pointer', marginTop: '5px', fontSize: '40px', }} />
                        </IconButton>
                        <div>
                            <Typography sx={{ fontSize: '20px', fontWeight: 'bold' }}>Your session has expired</Typography>
                            <Typography sx={{ fontSize: '20px', position: 'relative', top: '5px' }}>Please log in again to continue using the app</Typography>
                    ...
                    <DialogContentText sx={{ backgroundColor: '#ffffff', fontSize: '20px', fontWeight: 'bold', textAlign: 'center' }}>
                        <Link to="/login">
                            <Button sx={{ color: '#ba343b', fontSize: '15px', fontWeight: '600', border: '1px solid #ba343b', margin: '18px auto' }} variant="outlined"> Login</Button>
                        </Link>
                    </DialogContentText>
                </DialogContent>
            </Dialog>
